...
;; Centre initial frame
;; but now desktop-save-mode remembers frame sizes and all that
;; (when (display-graphic-p)
;;   (let* ((widthchars (cdr (assq 'width initial-frame-alist)))
;;          (width (* widthchars (window-font-width)))
;;          (left (truncate (/ (- (display-pixel-width) width) 2))))
...
(doom! :input
       ...
       ;;tree-sitter       ; syntax and parsing, sitting in a tree... (emacs <29 for now)
       ;;upload            ; map local to remote projects via ssh/ftp

       :os
       (:if (featurep :system 'macos))               ; improve compatibility with macOS
       ...
       (org +dragndrop +hugo +roam2)    ; organize your plain life in plain text
